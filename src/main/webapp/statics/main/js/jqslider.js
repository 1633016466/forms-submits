$.fn.jqslider = function() {
    var fun = function(parentObj){
        ...
        parentObj.hover(function(){
        	clearTimeout(timer);
            parentObj.find('.prev-btn').show(3000);
            parentObj.find('.next-btn').show(3000);
        },function(){
            parentObj.find('.prev-btn').hide(3000);
            parentObj.find('.next-btn').hide(3000);
        ...
        function DisplayPic() {
            ...
            parentObj.find('.list-box li').eq(hideIndex).css({'z-index':5}).stop(true,true).animate({'opacity':0}, 1000 , function(){
